...
function execute_text_area(id_of_area){
	  ...
	  for(var p =0;p<all_lines.length;p++){
		  ...
		  if(!listcontains(one_line, ["//","#"])&&one_line.length!==0){
			  ...
			  console.log("Time to parse this line(ms):"+time);
		  }
	  }
}
...
//	1 / (i z + 1)
	//console.log(simact.Algebrite.imag("1/(1+2*z*i)").toString());
	//console.log(simact.Algebrite.imag("(1/(1+1*i*z))").toString());
	
//	var str = "1 / (i z + 1)";
//	var denom = simact.Algebrite.run("ddd=(denominator("+str+"))").toString();
//	var denom2 = simact.Algebrite.run("ddd*conj(ddd)").toString();
//	var nom = simact.Algebrite.run("numerator("+str+")*conj(ddd)");
//	var res = simact.Algebrite.run("("+nom+")"+"/"+"("+denom2+")").toString();
//	console.log(denom2);
//	console.log(res);
}
